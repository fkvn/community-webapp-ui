...
function JobCard({ card = DEFAULT_CARD_INFO }) {
	...
	const jobInfo = { ...DEFAULT_JOB_CARD, ...info };
				...
				<Meta
					title={
						<Typography.Title level={5} className="text-white m-0 p-1" ellipsis>
							...
							>
								{jobInfo?.[`${EXPERIENCE_PROP}`]?.length > 0 && (
									<>
										{jobInfo?.[`${EXPERIENCE_PROP}`].toUpperCase()}
										{" - "}
									</>
								)}{" "}
	...
	const cover = (
		<div style={{ position: "relative" }}>
			<Carousel
				...
				autoplay
			>
				{jobInfo?.[`${PICTURE_LIST_PROP}`].map((img, idx) => (
					<div key={idx}>
						<div>
							{image({
								width: "100%",
						...
						onClick={() =>
							window.open(
								`https://www.google.com/maps/place/${
									jobInfo?.[`${LOCATION_PROP}`]?.[`${LAT_PROP}`]
								},${jobInfo?.[`${LOCATION_PROP}`]?.[`${LNG_PROP}`]}`,
								"_blank"
							)
						}
						ellipsis
					>
						{jobInfo?.[`${LOCATION_PROP}`]?.[`${ADDRESS_PROP}`]}
					</Typography.Text>
				</Col>
			</Row>
		</>
